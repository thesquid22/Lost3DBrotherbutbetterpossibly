...
static u32 set_mario_action_airborne(struct MarioState *m, u32 action, u32 actionArg) {
    ...
    switch (action) {
        ...
        case ACT_HOLD_JUMP:
            m->marioObj->header.gfx.animInfo.animID = -1;
            set_mario_y_vel_based_on_fspeed(m, 44.0f, 0.25f);
            m->forwardVel *= 0.8f;
            break;
        ...
        case ACT_SIDE_FLIP:
            set_mario_y_vel_based_on_fspeed(m, 42.0f, 0.0f); // 62.f
            m->forwardVel = 8.0f;
...
void debug_print_speed_action_normal(struct MarioState *m) {
	...
	if (gShowPosText){
        if (gPlayer1Controller->buttonDown | gPlayer1Controller->buttonPressed == L_TRIG) {	
