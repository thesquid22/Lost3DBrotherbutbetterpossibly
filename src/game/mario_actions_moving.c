...
s32 check_ground_dive_or_punch(struct MarioState *m) {
    UNUSED s32 unused;
	//print_text_fmt_int (0, 200, "%d", buffer);

    if (m->input & INPUT_B_PRESSED && !(m->flags & MARIO_METAL_CAP)) {
	...
	if (gPlayer1Controller->buttonDown == B_BUTTON && Dash_count < 62 && m->flags & MARIO_METAL_CAP) {
		if (buffer < 31)
		buffer++;
		if (buffer == 30)
        return set_mario_action(m, ACT_CHARGE, 1);
	} else if (gPlayer1Controller->buttonDown == B_BUTTON && buffer > 29 && m->flags & MARIO_METAL_CAP) {
		buffer=31;
...
s32 act_charge(struct MarioState *m) {
    ...
    Dash_count += 1;
    if (Dash_count < 100 & m->actionArg == 2) {
...
s32 act_death_exit_land(struct MarioState *m) {
    ...
    if (is_anim_at_end(m)) {
        set_mario_action(m, ACT_IDLE, 0);
        m->pos[0] -= 0.0f * sins(m->faceAngle[1]);
        m->pos[2] -= 0.0f * coss(m->faceAngle[1]);
