...
s32 act_holding_pole(struct MarioState *m) {
    ...
    if (m->controller->stickY > 16.0f) {
        ...
        if (poleBehavior == bhvTree && m->controller->stickY > 50.0f) {
...
s32 act_start_swinging(struct MarioState *m) {
    m->actionTimer++;
    ...
    set_mario_animation(m, ACT_SWINGING);
    play_sound_if_no_flag(m, SOUND_ACTION_HANGING_STEP, MARIO_ACTION_SOUND_PLAYED);
    update_hang_stationary(m);
...
s32 act_swinging(struct MarioState *m) {
    ...
    if (m->input & INPUT_NONZERO_ANALOG) {
        return set_mario_anim_with_accel(m, ACT_SWINGING, drag);
    }
...
s32 mario_execute_automatic_action(struct MarioState *m) {
    ...
    switch (m->action) {
        ...
        case ACT_START_SWINGING:         cancel = act_start_swinging(m);         break;
