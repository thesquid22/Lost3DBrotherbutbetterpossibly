...
void render_100_coin_star(u8 stars) {
    if (stars & (1 << 6)) {
        ...
        sStarSelectorModels[6] = spawn_object_abs_with_rot(gCurrentObject, 0, MODEL_STAR2,
...
void bhv_act_selector_init(void) {
    ...
    while (i != sObtainedStars) {
        if (stars & (1 << sVisibleStars)) { // Star has been collected
            selectorModelIDs[sVisibleStars] = MODEL_STAR2;
            i++;
...
void print_act_selector_strings(void) {
    ...
    if (sVisibleStars != 0) {
        ...
        print_menu_generic_string(actNameX, 67, selectedActName);
#endif
    }
